...
`docker-compose.yml` предназначен для серверного запуска:

- `backend` собирается в финальный образ и запускается без `go`
- `frontend` собирается в финальный образ и запускается без `vite`
