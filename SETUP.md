...
- для встраивания проекта в `iframe` оставьте `ALLOW_IFRAME_EMBED=true`; в продакшене вместо `IFRAME_ANCESTORS=*` лучше указать конкретный домен родительского сайта
