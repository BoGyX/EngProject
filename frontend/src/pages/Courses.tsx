...
import { Course, UserCourse, studyService } from '../services/studyService'
...
function CourseCard({ course, userCourse }: CourseCardProps) {
  ...
  const className = 'overflow-hidden rounded-lg border border-gray-200 bg-card-light shadow-md transition-all hover:border-link-light hover:shadow-lg'
        ...
        {userCourse && (
          <div className="mb-4 space-y-2">
            ...
            <div className="h-2 rounded-full bg-gray-200">
              <div className={`h-2 rounded-full transition-all ${progress >= 100 ? 'bg-green-500' : 'bg-slate-400'}`} style={{ width: `${progress}%` }} />
            </div>
          </div>
        )}
